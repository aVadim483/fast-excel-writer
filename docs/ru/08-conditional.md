## FastExcelWriter — Условное форматирование (since v.6.4)

Условное Formatting allows formatting options to be set based on the value of a cell.
...
to use the `applyУсловноеFormatting()` function, as this can negatively affect performance. 
Use `$sheet->addУсловноеFormatting($range, $conditional)` to set the same formatting conditions for a range of cells
...
Условное formatting of the dataBar type is used to display data bars within cells 
