...
Заранее вычисленным результатом может быть число, строка, логическое значение или значение ошибки
```php
$sheet->writeRow([
    ['=IF(A1>0, "PASS", "FAIL")', 'PASS'],  // строка
    ['=A1=A2', false],                      // логическое значение
    ['=A1/0', '#DIV/0!'],                   // ошибка
]);
```
...
во временный файл листа. Поэтому память не зависит от количества строк — в замере между 50 000
и 500 000 записанных строк роста не было вообще.

Всё, что нельзя сбросить вместе со строкой, остаётся в памяти до ```Excel::save()```:

| хранится в памяти до сохранения | примерно | примечания |
|---|---|---|
| ячейки ```Area``` | 0,3 КБ на ячейку | сбрасываются в ```Sheet::writeAreas()``` или ```Excel::save()``` |
| объединения ячеек | 0,5 КБ на объединение | |
| гиперссылки | 1 КБ на ссылку | не более 65 530 на лист (```Sheet::MAX_HYPERLINKS```) |
| заметки | 1 КБ на заметку | |
| правила проверки данных | 0,6 КБ на правило | одно правило может покрывать целый диапазон |
| правила условного форматирования | 0,5 КБ на правило | одно правило может покрывать целый диапазон |
| таблица общих строк | 0,5 КБ на уникальную строку | только при ```'shared_string' => true``` |
| диаграммы | всё описание диаграммы | значения, переданные массивом, тоже хранятся |

Исключение — изображения: содержимое картинки сразу пишется во временный файл, в памяти остаются
только её размеры и хеш, а одинаковые картинки попадают в файл лишь один раз.

Поэтому обычная выгрузка на миллионы строк почти ничего не стоит, а та же выгрузка, где **каждая**
строка получает ещё и заметку, гиперссылку или собственное объединение, растёт линейно. Если упёрлись
в лимит памяти, смотрите сначала в эту колонку: как правило, одно правило на диапазон заменяет
тысячи правил на отдельные ячейки.

```php
$conditional = Conditional::greaterThan(100, ['fill-color' => '#FFC7CE']);

// 100 000 правил в памяти
foreach ($rows as $i => $row) {
    $sheet->writeRow($row);
    $sheet->addConditionalFormatting('C' . ($i + 1), $conditional);
}

// тот же результат, одно правило
$sheet->addConditionalFormatting('C1:C100000', $conditional);
```

### Когда нужна область (Area) { #when-you-need-an-area }

Построчная запись идёт только вперёд — уже сброшенную строку изменить нельзя. Используйте
```Sheet::beginArea()``` / ```Sheet::makeArea()```, когда нужно

* записать значение в ячейку уже записанной строки;
* применить стиль к ячейкам предыдущих строк;
* заполнять ячейки в произвольном порядке (например, собирать оглавление по ходу записи данных).

Ячейки области живут в памяти до её сброса, поэтому делайте области настолько маленькими,
насколько позволяет задача — блок шапки или итоговая таблица, а не весь лист.

### Ограничения Excel

Некоторые ограничения формата обойти нельзя — файл, который их нарушает, Excel не открывает:

| ограничение | значение | поведение библиотеки |
|---|---|---|
| символов в ячейке | 32 767 | более длинное значение обрезается — так же поступает сам Excel при вводе |
| гиперссылок на листе | 65 530 | следующая ссылка вызывает исключение (```Sheet::MAX_HYPERLINKS```) |
| значащих цифр числа | 15 | при ```auto_convert_number``` более длинная числовая строка остаётся текстом |
| строк / столбцов листа | 1 048 576 / XFD | адрес за их пределами отклоняется |
