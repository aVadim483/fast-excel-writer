...
Формула правила может ссылаться на ячейку другого листа:
```php
$sheet->addConditionalFormatting('B2:B100', Conditional::greaterThan('=Limits!$A$1')->setFillColor('#FFC7CE'));
```
Обычный элемент ```<cfRule>``` такие ссылки не поддерживает, поэтому такое правило записывается
в x14-расширение листа (стиль при этом встраивается прямо в правило) — так же, как это делает сам Excel.
