...
**ПРИМЕЧАНИЕ:** Apache OpenOffice может отображать изображения со сдвигом на несколько
пикселей вверх относительно ячейки привязки
(см. [issue #121](https://github.com/aVadim483/fast-excel-writer/issues/121)).
Генерируемый XML соответствует спецификации OOXML — MS Excel и LibreOffice отображают
тот же файл корректно — то есть сдвиг вызван особенностями позиционирования изображений
в самом OpenOffice и не может быть исправлен на стороне библиотеки. В качестве обходного
пути можно компенсировать сдвиг смещениями 'x' / 'y', но учтите, что эти смещения
применяются во всех программах просмотра, включая MS Excel.
