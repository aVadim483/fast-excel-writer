## FastExcelWriter — Листы

### Запись данных Row by Row vs Direct
...
### Запись данных Cell Values

Usually, values is written sequentially, cell by cell, row by row. Запись данных to a cell moves the internal pointer
...
You can use simple and advanced formats. Пример of setting the format for each cell:
...
Запись данных rich text to a cell can be done using RichText instances. Here's an example, which creates the following rich text string:
